...
let timesAddedToFav = []
for(const uniqueSound of uniqueSounds){
    let count = 0;
    for(const user of users){
        for(const sound in user.favoritesSounds){
            if(uniqueSound == sound){
                count ++
            }
            
        }
    }
    timesAddedToFav.push(count)
}
for(let i = 0; i < uniqueSounds.length; i++){
    console.log(`The sound: ${uniqueSounds[i]} was added to favorites ${timesAddedToFav[i]} times`)
}
